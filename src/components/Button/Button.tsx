...
> = ({ children, onClick, className, disabled, hide, type, loading, kind = "normal", size = "normal" }) => {
  if (hide) {
    return null;
  }
  return (
    <button
      ...
      className={cn(
        ...
        className,
        {
          ...
          "text-gray-400 bg-gray-200 hover:bg-gray-200 active:bg-gray-200": loading && kind === "normal",

          "text-white bg-blue-200 hover:bg-blue-200 active:bg-blue-200": loading && kind === "success",

          "text-gray-400 bg-red-200 hover:bg-red-200 active:bg-red-200": loading && kind === "confirm",
        },
      )}
      onClick={onClick}
