...
export function TransferDialog() {
  ...
  return (
    <>
      ...
      <dialog open={isOpen}>
        <article>
          <a href="#close" aria-label="Close" className="close" data-target="modal-example" onClick={() => setIsOpen(false)}></a>
...
function TransferDialogForm({ onCloseDialog }: TransferDialogFormProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const transferAddress = (document.getElementById("transfer-address") as HTMLInputElement)?.value;
      ...
      const transferAmount = parseFloat((document.getElementById("transfer-amount") as HTMLInputElement)?.value);
      ...
      const precision = 5;
      const amount = new BN(transferAmount * 10 ** precision).mul(new BN(10).pow(new BN(tokenDecimals - precision)));
    ...
    <>
      ...
      <form>
        <label htmlFor="transfer-address">
          ...
          <input type="text" id="transfer-address" name="transfer-address" placeholder="5FE51…xe1XL" required />
        </label>

        <label htmlFor="transfer-amount">
          Amount ({tokenSymbol})
          <input type="number" id="transfer-amount" name="transfer-amount" placeholder="10.0" min={0} required />
        </label>
      </form>
      {/* Footer */}
