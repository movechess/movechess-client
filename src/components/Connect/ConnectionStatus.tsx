...
export function ConnectionStatus() {
  ...
  return (
    <article>
      ...
      <p>
        {isConnected ? (
          <code style={{ color: "rgba(56, 142, 60, 1)", background: "rgba(56, 142, 60, .1)" }}>Connected</code>
        ) : error?.message ? (
          <code style={{ color: "rgba(198, 40, 40, 1)", background: "rgba(198, 40, 40, .1)" }}>{error.message}</code>
        ) : (
      ...
      {isConnected && (
        <>
          ...
          <small>
            <p>
              {activeChain?.name} {!hasPalletContracts && <span style={{ color: "rgba(198, 40, 40, 1)" }}>(pallet-contracts not found)</span>}
            </p>
          </small>
